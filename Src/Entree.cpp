...
static void SignalVoitureGaree ( int noSignal )
// Algorithme :
//
{
	int pid;
...
static void LanceRequete(StructRequete req) 
// Algorithme :
//
{
...
static void Initialisation(int _numero, int _idBal, int _idSemaphoreSynchro, int _idSemaphoreContenuParking, int _idSemaphoreRequete, int _idSemaphoreNbPlaces , int _idMPContenuParking, int _idMPRequete, int _idMPNbPlaces)
// Algorithme :
{
...
// Algorithme :
{
	int index = 0;
 	...
 	{	StructRequete req;
 		...
 		index = getVoiturier();
 		if(index == -1)
	 	{	Afficher(TypeZone::MESSAGE, "Erreur, aucun voiturier");
	 	}
	 	else 
	 	{	//On se sert de numéro place de Voiturier pour stocker temporairement le pid du voiturier
	 		if(restePlace()) 
...
//
{
	Initialisation(_numero, _idBal, _idSemaphoreSynchro, _idSemaphoreContenuParking, _idSemaphoreRequete, _idSemaphoreNbPlaces , _idMPContenuParking, _idMPRequete, _idMPNbPlaces);
 	Moteur();
} //----- fin de Entree
